...
export default function Sidebar() {
    ...
    const showCategories = categoriesArr.map((el, index) =>
        <div
            key={index}
            className="w-[80%] mx-auto px-1 py-2.5 mb-3 rounded-lg hover:bg-red-800 font-medium text-sm cursor-pointer"
        >
            {el.name}
        </div>);

    return (
        <aside className="w-[15%] h-[100vh] sticky top-0 bg-[#141313] text-center">
            <Link href="/">
